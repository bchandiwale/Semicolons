...
export class PatientComponent implements AfterViewInit {
  displayedColumns: string[] = ['ID', 'NAME', 'WEIGHT', 'AGE', 'GENDER', 'actions'];
  gridColumns = 3;
  allpatientData!: any;
  dataSource!: any;
  admissionTypeCount!: any;
  currentCount = 10;
  currentCountElective = 10;
  currentCountUrgent = 10;
  ...
  ngOnInit() {
    this.patientService.getAdmissionTypeCount().subscribe((res) => {
      ...
      this.currentCount = this.admissionTypeCount.Normal;
      this.currentCountElective = this.admissionTypeCount.Elective;
      this.currentCountUrgent = this.admissionTypeCount.Emergency;
    ...
    // setInterval(() => {
    //   if (this.currentCount < this.admissionTypeCount[0].Elective) {
    //     this.currentCount++;
    //   }
    // }, 0);
    // setInterval(() => {
    ...
    // setInterval(() => {
    //   if (this.currentCountUrgent < this.admissionTypeCount[0].Elective) {
